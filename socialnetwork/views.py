...
def AddPost(request):
    ...
    context = {}
    errors=[]
    entry = Entry.objects.get(created_by = request.user)
    form = EditForm( instance = entry) 
...
def confirm_registration(request, username, token):
    ...
    new_user.save()    
 # entry = Entry(picture = "images/default.jpg", age=0 ,created_by=new_user,update_time=datetime.now(),creation_time= datetime.now())
    # entry = Entry(age=0 ,created_by=new_user,update_time=datetime.now(),creation_time= datetime.now())
    # entry.save()
...
def ViewProfile(request,id):
    ...
    entry = get_object_or_404(Entry,created_by= target)
...
def add_comment(request,id):
    ...
    for item in comments:
        ...
        entry = get_object_or_404(Entry,created_by = comment.user)
